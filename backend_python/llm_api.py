...
from ppt_engine_v2 import export_to_ppt, SlideDesigner  # 升级为基于 SVG 的新引擎
...
- "color_scheme" 是全局配色方案名。可选值： "mckinsey_blue" (商务科技), "deloitte_green" (农业自然), "bcg_purple" (人文艺术)。请根据主题智能选择。
...
=== 页面属性赋值指南 ===
- color_scheme: "mckinsey_blue" (默认), "deloitte_green" (农业环保), "bcg_purple" (人文艺术)
...
def _add_svg_previews(slides_data):
    """为 slides_data 中的每一页生成 SVG 预览"""
    if not isinstance(slides_data, dict):
        return slides_data
        
    color_scheme = slides_data.get("color_scheme", "mckinsey_blue")
    slides = slides_data.get("slides", [])
    
    designer = SlideDesigner(color_scheme=color_scheme)
    total = len(slides)
    
    for i, slide in enumerate(slides):
        try:
            # 生成 SVG
            svg = designer.generate_slide_svg(slide, i + 1, total)
            slide["preview_svg"] = svg
        except Exception as e:
            print(f"⚠️ 生成第 {i+1} 页预览图失败: {e}")
            slide["preview_svg"] = ""
...
def _tool_generate_ppt(topic, pages=3):
    ...
    try:
        ...
        # 生成预览图
        slides_data = _add_svg_previews(slides_data)
...
def _tool_modify_ppt(feedback):
    ...
    if intent in ['delete', 'reorder']:
        ...
        final_full_data = _add_svg_previews(final_full_data)
    ...
    if updated_data is not None:
        # 如果模型返回了嵌套对象，则提取 slides；否则视为 slides 数组
...
def _tool_export_ppt():
    ...
    print(f"  🖼️ 正在为 {len(slides)} 页幻灯片生成封面图片...")
    for i, slide in enumerate(slides):
        slides[i] = generate_cover(i + 1, slide)
    ...
    # export_to_ppt 已经更新过，可以直接接受 full_data 或 slides 列表
    export_to_ppt(full_data, output_file)
